...
from model import MaskedNLLLoss, DialogueRnn
...
def train_or_eval_model(model, loss_function, dataloader, epoch, optimizer=None, train=False):
    ...
    with tqdm(dataloader) as td:
        for data in td:
            ...
            textf, visuf, acouf, party_mask, mask, label = [d.cuda() for d in data[:-1]] if args.cuda else data[:-1]
            
            log_prob = model(textf, visuf, acouf, party_mask, mask)
...
def parse_args():
    ...
    # model settings.
    parser.add_argument('--context_attention', type=str, default='general')
    parser.add_argument('--attn_on_emotion', type=bool, default=True)
    ...
    parser.add_argument('--emotion_state_dim', type=int, default=300)
    parser.add_argument('--hidden_layer_dim', type=int, default=300)
    parser.add_argument('--concat_attn_dim', type=int, default=100)

    parser.add_argument('--dropout', type=float, default=0.1)
    parser.add_argument('--rec_dropout', type=float, default=0.25)
...
if __name__ == '__main__':
    ...
    for seed in [1, 11, 111, 1111, 11111]:
        ...
        model = DialogueRnn(args)
        print('IEMOCAP DialogueRnn MODULE ...')
        ...
        lf = open('logs/dialoguernn_iemocap_logs.txt', 'a')
        ...
        rf = open('results/dialoguernn_iemocap_results.txt', 'a')
